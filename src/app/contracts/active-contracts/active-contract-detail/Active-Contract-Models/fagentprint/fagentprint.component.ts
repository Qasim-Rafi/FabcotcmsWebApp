...
import { ChangeBankAccountComponent } from 'src/app/billing-and-payment/active-bills/open-active-bill/change-bank-account/change-bank-account.component';

@Component({
  selector: 'app-fagentprint',
  templateUrl: './fagentprint.component.html',
  styleUrls: ['./fagentprint.component.css']
})
export class FagentprintComponent implements OnInit {
  @Input() ContractId;
  queryParems: any = {};
  bill_id: any = {};
  response: any;
...
billList:any = {};
  ...
  getallbills() {
    ...
    {
    this.billList=this.response.data.objList;
       let billId  = this.billList.filter(x=>x.contractId == this.ContractId)

    this.bill_id = billId[0].id;
      this.fetch((data) => {
        this.rows = data;
  ...
  fetch(cb) {
    ...
    this.http
    .get(`${environment.apiUrl}/api/BillingPayments/GetContractBillByIdForAgentCommissionHometextile/` + this.bill_id)
    .subscribe(res => {
      this.response = res;
